...
"#;
    ...
    let html_output = run_command(&[
        ...
        "--css", css_file.to_str().unwrap(),
        "--js", js_file.to_str().unwrap(),
    ]);
    ...
    assert!(html_content.contains("<style>"), "CSS was not embedded in HTML");
    assert!(html_content.contains("<script>"), "JavaScript was not embedded in HTML");
    assert!(html_content.contains("font-family: Arial"), "CSS content is incorrect");
    assert!(html_content.contains("document.addEventListener"), "JS content is incorrect");
    ...
    let slides_output = run_command(&[
        ...
        "--width", "1024",  // Widescreen format
        "--height", "768",
        "--base-name", "presentation",  // Custom base name
        "--format", "png",
    ]);
    ...
    let pptx_output = run_command(&[
        ...
        "--title", "Big Slides Demo",
        "--pattern", "presentation_*.png",  // Match our custom naming
    ]);
    ...
    for file in essential_files {
        assert!(archive.by_name(file).is_ok(), "Missing essential file in PPTX: {}", file);
    }
...
fn test_pipeline_with_empty_content() {
    ...
    assert!(slides_output.status.success(), "generate-slides command failed");
